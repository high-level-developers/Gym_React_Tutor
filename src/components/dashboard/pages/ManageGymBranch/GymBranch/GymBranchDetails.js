...
const GymBranchDetails  = (props) => {
    ...
    const [visitCost, setVisitCost] = useState(state.content.visitCost? state.content.visitCost: '');
    const [attendanceCost, setAttendanceCost] = useState(state.content.attendanceCost? state.content.attendanceCost: '');
    ...
    const onSave = () => {
        setLoading(true)
        if(name === '' || owner ==='' || membership === '' || visitCost === '' || attendanceCost === ''){
            setNameError(name === '');
            setVisitError(name === '');
            setAttendError(name === '');
        ...
        updateGym(gymData.qrCode, name, owner, "membership", visitCost, attendanceCost)
        .then(data => {
            if(data.success === 'success'){
                getGym(gymData.qrCode)
                .then(data=>{
       ...
       <Container>
           ...
           <div className="content">
                ...
                <div className="info-container" style={{padding:'0px', width: 'calc(100% - 5px)'}}>
                    ...
                    { role.includes(3) &&  <React.Fragment>
                        <span className="branch_title">Branch Information</span>
                        ...
                        <div className="flex" style={{marginBottom:'10px'}}>
                            ...
                            <span className="content-text">{state.content.membership}</span>
                        </div>
                        <div className="flex" style={{marginBottom:'10px'}}>
                            <span className="label">
                                {t('Visit cost')}
                            </span>
                            <span className="content-text">{visitCost}</span>
                        </div>
                        <div className="flex" style={{marginBottom:'10px'}}>
                            <span className="label">
                                {t('Attendance cost')}
                            </span>
                            <span className="content-text">{attendanceCost}</span>
                        </div>
                        </React.Fragment>
                    }
                    { role.includes(4) &&  <React.Fragment>
                        <span className="branch_title">Branch Information</span>
                        <span className="label">
                            {t('GYM name')}
                        </span>
                        ...
                        <input className={visitError? "input error": "input"}
                            value={visitCost}
                            onChange={e => {
                                setVisitError(false);
                        ...
                        <input className={attendError? "input error": "input"}
...
export default GymBranchDetails;
